...
| モード | 動作 | コスト |
|--------|------|--------|
| `inject`（既定） | 新しい diff ごとに 1 回ブロックして **プロパティ・チェックリスト** を注入。動いているエージェントが対象ファイル（この作業ツリーの未コミット変更）を各プロパティで検証し、満たさないものを直してから完了。検査対象は `git::changed_files` の和集合であり、propguard は各ファイルの作成者を検証していないため、人間や別セッションの変更も含まれうる。 | 無料（追加プロセス無し） |
