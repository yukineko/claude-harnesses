...
| `condukt state check-criteria --run <id> --task <id>` | run the mechanical gate for a task's `done_criteria`: exits 0 when met (or non-mechanical), 1 when it fails. The JSON `skip_verifier` field is true ONLY when all three vetoes clear — the criteria is non-behavioral, a command could be extracted, AND that one command covers the whole criteria (`coverage: Single`) — and that command then passes. A criteria that asserts more than the single extracted command can check (an enumerated list, or several named commands) reports `coverage: Partial` and `skip_verifier:false`, so a partial check is never reported as a complete pass; a `mechanical_check` hint does not exempt it. Behavioral criteria always report `skip_verifier:false`. |
| `condukt state conflict-check/abandon/list-tasks/cancel/pause/resume` | cross-session safety + run editing: detect file/goal conflicts before `init`, return stuck `running` tasks to `pending` (`--all-stuck`), list/cancel a run's tasks, pause/resume a conflicting run (see the skill's Phase 0/3.5 and the cancel utility). **`--all-stuck` is tri-valued**: every Running task is stuck, healthy, or UNDETERMINED, and the undetermined ones are always printed on stderr with their id/class/reason — never folded into a bare `nothing to abandon`. It **exits 3** when a task is `unobservable` (a durable progress signal could not be read at all: no `updated_at`, an unreadable worktree HEAD, no worktree), and **exits 0** when nothing was unobservable — including when the only undetermined tasks are `awaiting-sample` (every signal read cleanly, the multi-sample window just needs another observation, which is what the first invocation always is). `awaiting-sample` means "unknown right now", not "healthy"; it is reported, bounded by the freeze window, and self-resolving on the next call. |
| `condukt state claim/release/heartbeat/claims` | **cross-session file-claim registry** (`<state>/<project>/claims.json`) that turns `conflict-check`'s one-time advisory snapshot into a live, enforcing lease so two sessions on the same machine never process the same work. `claim --run <id> [--file ...]` leases a task's `touched_files` (defaults to the run's whole decomposition); it **hard-skips** (exits 1, prints the conflicting holders) any file a *live* holder from another run owns. `release` frees them (all, or `--file`), `heartbeat` refreshes liveness so a busy session isn't reaped, `claims` prints the live registry. Enforcement is automatic: `state set --status running` auto-claims the task's files and **exits 1 with a skip JSON** on a live conflict; terminal transitions auto-release. Stale claims (heartbeat older than the stuck-TTL) are reaped — liveness is anchored to the heartbeat, not the ephemeral CLI pid; every op is fail-soft and serialized by the per-run lock. **Keyed by the MAIN worktree root**, so every linked worktree of a repo (and the main tree) reads and writes the SAME `claims.json` and contends on the SAME `__claims__.lock` — under CLAUDE.md §8 each session works in its own linked worktree, which is exactly where the lease has to hold. Migration: claims written before this under the old per-worktree key are not moved; they are orphaned and age out via the heartbeat TTL. Run state, precedent and autoflow's stores still key off `repo_root` and are unchanged (backlog `43393ce2`). |
| `condukt state autonomy-check [--explain]` | report whether condukt is in autonomous mode. Resolution order, highest first: env `HARNESS_AUTONOMOUS`, env `CONDUKT_AUTONOMOUS` (legacy alias, still honoured), the shared switch file written by `autonomy-set`, config `autonomous`, then off. Prints exactly `{"autonomous":<bool>}` (a frozen byte contract — `--explain` is where `"source"` is added, never the plain form) and exits 0 when autonomous, 1 when not, so the skill can deterministically degrade human gates (e.g. the Phase 3 agreement) only when autonomous. A switch file that exists but cannot be read/parsed is NOT autonomous (fail-closed) and prints a warning naming the file on stderr; an ABSENT switch file is a plain "never set" and is silent. Off by default (every `AskUserQuestion` still fires — backward compatible). |
| `condukt state autonomy-set on\|off` | durably set the SHARED autonomy switch for this repo: writes `<$HARNESS_AUTONOMY_DIR or ~/.harness/autonomy>/<project-key>.json` atomically (temp + rename). The key comes from the repo's MAIN worktree root, so a switch set here is visible from every linked worktree (CLAUDE.md §8), and ctxrot / autoflow read the same file DIRECTLY — no subprocess, so a missing binary on `PATH` can never masquerade as "not autonomous". |
| `condukt state autonomy-path` | print `{"path":"<switch file>","source":"<deciding layer>"}` and exit 0. The path is where `autonomy-set` writes, whether or not it exists yet. |
...
autonomous     = false                    # when true, degrade human gates (Phase 3 agreement) to deterministic defaults.
                                          #   Lowest layer but one: the shared switch file (condukt state
                                          #   autonomy-set) overrides it, and HARNESS_AUTONOMOUS /
                                          #   CONDUKT_AUTONOMOUS override that.
...
| `CONDUKT_AUTONOMOUS` | `false` | Set to `1`/`true` to run autonomously (degrades human gates). Overrides BOTH config `autonomous` and the shared switch file, so a switch left on can always be forced off for one invocation. Read by `state autonomy-check`. |
| `HARNESS_AUTONOMOUS` | unset | The harness-wide spelling of the same override, read by condukt, ctxrot and autoflow alike. Outranks `CONDUKT_AUTONOMOUS`. |
| `HARNESS_AUTONOMY_DIR` | `~/.harness/autonomy` | Directory holding the per-project autonomy switch files (`<project-key>.json`). Mainly for tests, which must never touch the real one. |
...
| `CONDUKT_STUCK_TTL_SECS` | `1800` | Age (seconds) past which a `running` task becomes a CANDIDATE for `state abandon --all-stuck`. TTL-staleness is necessary, not sufficient: the bulk path abandons only a task whose progress is a confirmed `Known(Stalled)` (backlog `356bd51d`). Everything it could NOT determine is reported on stderr and split by cause — `unobservable` (exit 3) vs `awaiting-sample` (exit 0, observation incomplete) — rather than silently counted as healthy (backlog `b637936f`). |
