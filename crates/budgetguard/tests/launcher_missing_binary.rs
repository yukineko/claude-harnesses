...
//! `bin/budgetguard` (the POSIX launcher) — what happens when NO per-platform
...
//! missing build made budgetguard silently certify every stop: the turn ended as
//! if `budgetguard gate` had run and found nothing to block (CLAUDE.md §1/§3 —
...
fn stop_gate_missing_binary_blocks_instead_of_silently_certifying() {
    ...
    assert!(
        reason.contains("budgetguard") && reason.to_uppercase().contains("NOT"),
...
fn cli_subcommands_exit_nonzero_rather_than_printing_an_empty_result() {
    for args in [vec!["status"], vec!["init"], vec!["install"], vec![]] {
        ...
        assert_ne!(
            r.code,
            0,
            "`budgetguard {}` must not report success it did not earn: {r:?}",
            args.join(" "),
        );
    }
}
