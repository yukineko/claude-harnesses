...
- **`.githooks/`** — `git config core.hooksPath .githooks` で有効化する versioned な git hooks。`pre-commit` は stdlib-only python3 のブロッキングゲート群（injectguard / fail-open-guard / doc-claims / test-weakening / version-lockstep / bump-on-change / secret-guard / raw-io-ratchet）＋ Rust 製 main-tree-guard を commit 時に走らせ、非0終了は commit を止める（fail-closed。`.githooks/pre-commit` 冒頭コメント参照）。`pre-push` は GATE_CRATES（blastguard/propguard/specguard/stuckguard/taintguard/mutategate/overwatch/parallelguard）への変更を検知して Continuous-Audit ラウンド実行（`scripts/continuous-audit.sh --dry-run`）を勧める、advisory な fail-soft（常に exit 0、push を止めない）。両者を混同しない: pre-commit はブロッキング、pre-push はアドバイザリ。
...
| parallelguard | always-on | 1 セッション内の同時実行数を binary が数えて強制するゲート（GATE crate）。PreToolUse で in-flight な `Bash`（shell プール）と `Task`/`Agent`（subagent プール）を数え、上限（既定それぞれ 3。`HARNESS_MAX_PARALLEL` で下げられるが上げられない）を超える呼び出しを deny する。PostToolUse で slot を返し、ターン境界（SessionStart/UserPromptSubmit/Stop）で台帳を消す。プールを 2 本に分けるのは共有 1 本が deadlock するため（subagent は生存期間ずっと slot を持つので、上限数の subagent が live だとその subagent 自身の `Bash` が全部 deny される）。判定不能（payload 不正・台帳読めない・lock 取れない・書き込み失敗・panic・binary 欠損）は deny （PreToolUse の「無出力 exit 0」は allow そのものなので、沈黙は degrade として使えない） |
