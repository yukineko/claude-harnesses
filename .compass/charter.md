...
【2026-08-06 の裁定】上の (a) blastguard 監査の完遂 と (b) 横断パターンの一括是正 は、どちらを取るかを人間へ返した結果、**どちらも今回の一手には選ばれなかった**。選ばれたのは harness_core の型封印（DoD11）である — (a) と (b) はいずれも fail-open を 1 件ずつ発見して潰す経路であり、型封印はその種類の erasure をそもそも書けなくする側だから、というのが選定理由。(a) と (b) は棄却ではなく保留であり、次サイクルの候補として残る。この裁定は自動解決ではなく AskUserQuestion で人間が下した（compass の『矛盾は人間が裁く』に従う）。
