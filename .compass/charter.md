...
## current_gap
DoD9 は 5 of 22（propguard の監査が 2026-08-04 に landed し分子が 4→5。GATE crate としては 2 本目）。ゴールとの最大の差は『verdict 経路を持つと分かっているのに、一度も列挙されていない crate が 17 本ある』こと。分母は 2026-07-31 に定義済み（harness_core::verdict 参照クレート、実測 22）で単調増加が計算できる。propguard の監査は 39 サイトを未分類 0 で分類し、fail-open を 3 件発見した — F-1（filter list の部分ロードが検査対象を黙って狭める）は同コミットで修正、F-2（diff_text が git 失敗を消し空 diff が allow になる、backlog 1217fa4f）と F-3（mode の typo が独立検査を黙って降格、backlog 3ca750b9）は証拠付きで起票した。F-1 は blastguard の diffrisk.rs → exclude.rs に続く同一形の三例目で、ミラーギャップがまだ収束していないことを示す。次の右サイズは stuckguard（6988 行、GATE crate で最小の未監査）。ただし blastguard は分類が未完のまま据え置かれており（120 サイト中およそ 118 が個別未分類）、カテゴリ棄却が実在の欠陥を隠した当事者でもあるので、stuckguard の次はそこへ戻る。
