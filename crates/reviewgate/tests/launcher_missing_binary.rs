...
//! `bin/reviewgate` (the POSIX launcher) — what happens when NO per-platform
...
//! missing build made reviewgate silently certify every stop: the turn ended as
//! if `reviewgate review` had run and found nothing to block (CLAUDE.md §1/§3 —
...
fn stop_gate_missing_binary_blocks_instead_of_silently_certifying() {
    ...
    assert!(
        reason.contains("reviewgate") && reason.to_uppercase().contains("NOT"),
...
fn cli_subcommands_exit_nonzero_rather_than_printing_an_empty_result() {
    for args in [
        ...
        assert_ne!(
            r.code,
            0,
            "`reviewgate {}` must not report success it did not earn: {r:?}",
            args.join(" "),
        );
    }
}
