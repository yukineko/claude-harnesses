...
- **`condukt -.-> compass`** — Phase 0-next reads `compass gap` to pick the next
  move (`crates/condukt/skills/condukt/SKILL.md:167` "COMPASS_GAP=$(compass gap"). Combined with the forward
  `compass --> condukt` handoff, this forms a **2-cycle `condukt ↔ compass`**.
- **`condukt -.-> backlog`** — Phase 0-next reads it at
  `crates/condukt/skills/condukt/SKILL.md:164` "backlog list --status pending". `backlog` calls no one, so this adds **no** cycle on its own,
...
> `crates/condukt/skills/condukt/SKILL.md:988` "tdd/specguard を経路面") — that mention is a
...
> `crates/condukt/skills/condukt/SKILL.md:904` "tdd oracle --task". See the scope limit below.
...
- **`tdd`** is a leaf **under this doc's edge rule only** (SKILL/agent-file
  ...
  `crates/condukt/skills/condukt/SKILL.md:988` "tdd/specguard を経路面" and issues no bash call to it —
