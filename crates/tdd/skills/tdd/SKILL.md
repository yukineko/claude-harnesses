...
- Stop でブロックされる → テストが付いていない実装がある。テストを足す。純粋な
  リファクタ/リネーム/ドキュメントのみなら `tdd skip --reason "<理由>"` を打つ
  (理由必須・発行したセッション限定・1回だけ消費・発行と消費をゲートログに記録)。
  旧来の project root の `.tdd-skip` ファイルは撤去済み(共有ツリーの1回限りマーカーは
  次に停止した別セッションが消費してしまうため — CLAUDE.md §5)。
