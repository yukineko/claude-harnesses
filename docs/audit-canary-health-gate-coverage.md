...
|---|---|---|
| Copy each plugin in the stage | `scripts/rollout-plugins.sh:882` | `      canary_copy_row "$pn_row"` |
| Collect registry args if the row needs a registry update | `scripts/rollout-plugins.sh:884-886` | ```      if [ "$needs_registry" = "1" ] || [ "$force" = 1 ]; then\n        stage_reg_args+=("$name" "$version" "$target")\n      fi``` |
| Batch-commit the stage's registry pointer (dry-run variant) | `scripts/rollout-plugins.sh:888-890` | ```    if [ "${#stage_reg_args[@]}" -gt 0 ]; then\n      if [ "$dry" = 1 ]; then\n        registry_patch "$REGISTRY" "$OWNER" "$GIT_SHA" --dry-run "${stage_reg_args[@]}" | sed 's/^/  /'``` |
| Batch-commit the stage's registry pointer (real, non-dry-run) | `scripts/rollout-plugins.sh:891-892` | ```      else\n        registry_patch "$REGISTRY" "$OWNER" "$GIT_SHA" "${stage_reg_args[@]}" | sed 's/^/  /'``` |
