        ...
        loop {
            match std::fs::hard_link(&tmp_path, &path) {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => {
                    // Someone holds the lock. Reap ONLY on a positive "the
                    // owner is gone" (`Known(false)`). A live owner
                    // (`Known(true)`) AND an UNDETERMINED liveness (`kill`
                    // unspawnable / signalled — "I could not ask the OS")
                    // both fall through to the wait arm below. Reaping on
                    // "cannot tell" would steal a live holder's lock — the
                    // exact fail-open this lock exists to close.
