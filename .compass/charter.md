...
## current_gap
DoD9 は 4 of 22（mutategate の監査が 2026-08-04 に landed し分子が 3→4。GATE crate としては最初の1本）。ゴールとの最大の差は『verdict 経路を持つと分かっているのに、一度も列挙されていない crate が 18 本ある』こと。分母は 2026-07-31 に定義済み（harness_core::verdict 参照クレート、実測 22）で単調増加が計算できる。勾配は DoD9 のままであり、次の一手は 18 本の未監査 crate の先頭を1本削ること。GATE crate のうち監査済みは mutategate の1本のみ（2026-08-04）。残る propguard / stuckguard / specguard / overwatch と、分類が未完のまま据え置いた blastguard から先に取る。次の右サイズは propguard（7450 行）。最小は stuckguard（6988 行）だが、propguard は即時 block する gate で stuckguard は protect-the-protector （docs/gate-taxonomy.md）なので、同じコストなら勾配の大きい propguard を先に取る。なお 2026-08-02 に specforge の出口（ratified spec の requirement を backlog に積む、merge bc022557）が landed したが、これは source→executor の導線であって DoD9 の分子ではない — 監査ドキュメントの無い crate は監査済みと数えない。
