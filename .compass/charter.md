...
## definition_of_done
- [達成 2026-07-22 測定点75b91230] pre-pushのrollout-drift検査(scripts/check-plugin-rollout.pyのexit 1、「committed but not rolled out」サブクラス)が、pushをblockするよう動作を変更済み。使い捨てworktreeで、shipを0.1.8→0.1.9へ人為的にdriftさせた状態からのgit pushが非0(exit 1)で拒否されることを実測で確認し、driftなし状態はexit 0のままであることも確認した。
- [達成 2026-07-22] このrepoのローカルcloneでcore.hooksPathが.githooksに設定済み(git config core.hooksPath .githooksを実行、出力で確認)。
- [達成 2026-07-22 測定点75b91230] pre-pushの他の3チェック(GATE crate示唆・autonomy chain示唆・chronically-red CI)と、scripts/check-plugin-rollout.pyの他の2サブクラス(exit 2 enablement・exit 3 unverifiable)は既存どおりadvisoryのまま維持し、blockingに変えたのはrollout-drift本体(exit 1)のみであることをdiffで確認済み(スコープを広げすぎない)。
- [達成 2026-07-22 測定点75b91230] .githooks/pre-push側のコメントで、rollout drift検出時にpre-pushへ非0を返してpushを止める契約を明文化済み(スクリプトのコメント/ドキュメントが実装と一致)。
...
- pre-pushのchronically-red CI検査が実際にsemver-checks/build-and-commit-plugin-binariesの2workflowを継続的赤と報告している(2026-07-22実測)。これはadvisory範囲内の別課題であり、今回のnorth_starでは対象外。次のnorth_star候補になりうる。
