...
//! `unclaimed_worktree_with_advancing_activity_is_live`,
...
//! still reach `removable == true`; the next two are the control that the
//! `live` verdict is reached by observing something, on the claimed path and on
//! the unclaimed-witness path respectively. The last keeps the death rule's
//! session-registration term from being ignored outright: it flips one field
//! between two otherwise identical worktrees and requires the verdicts to
//! differ.
