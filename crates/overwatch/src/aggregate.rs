...
pub fn build(cwd: &Path) -> ProgressView {
    ...
    //
    // `store::load_leases` already separates "absent" (`Ok(empty)`) from
    // "present but corrupt / unreadable" (`Err`) via `boundary::read_to_string`'s
    // `Determination`. Binding it with `if let Ok(..)` — as this did — threw that
    // distinction away and left `sessions` empty for BOTH, which renders as
    // `(none)` = "no other session is live". That is the single most load-bearing
    // fact in this output: CLAUDE.md §8 says to assume another session always
    // exists, and condukt's main-tree guard reads this roster as a liveness input
    // before allowing a commit in main's shared working tree.
    match store::load_leases(cwd) {
        Ok(mut leases) => {
            store::reap_stale(&mut leases, now);
            view.sessions = roster_from_leases(&leases, now);
        }
        Err(e) => view.mark_undetermined(
            SOURCE_SESSIONS,
